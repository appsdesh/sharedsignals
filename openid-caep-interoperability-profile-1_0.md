...
OAuth server could be a separate entity other than the SSF Transmitter. SSF Transmitter acts as a resource server.
...
### OAuth Scopes
Depending on the features supported by the OAuth service and the SSF APIs, the OAuth client/SSF receiver SHALL discover the OAuth scopes as follows:
...
Upon discovery of the scopes by the the OAuth client/SSF receiver, the receiver MUST obtain an Access token with the discussed scopes from the OAuth Service. This access token should be used to authenticate and authorize SSF APIs at the transmitter.
